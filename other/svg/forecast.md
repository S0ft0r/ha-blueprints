...
Некоторые элементы интерфейса не умеют напрямую работать с `entity_picture` - им нужна строка с названием изображения - поэтому, чтобы использовать анимированный svg приходится использовать другой подход, а именно реализовывать вывод иконки при помощи `card_mod`. Ниже приведен пример, в котором используются, кроме погодной иконки, еще и иконки [жалюзи](./blind.md) и [солнечного компаса](./compass.md)
